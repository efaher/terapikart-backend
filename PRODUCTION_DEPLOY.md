...
6. `/health` yanıtında `persistentAccounts: true` görülmeden ilerleme.
...
4. Yeniden deploy sonrası `/health` ve giriş/oda smoke testi tekrarlanır.
...
6. Danışan bağlantısı ikinci cihazda açılır ve realtime seçim doğrulanır.
...
- `ADMIN_LICENSE_SECRET` kullanıcıya, müşteriye veya frontend'e verilmemeli.
